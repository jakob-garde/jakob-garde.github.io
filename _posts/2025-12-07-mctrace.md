...
*A few years ago I worked on the open-source software package McStas at DTU Physics (2015-2020). Recently I have been re-visiting the software package as a fun hobby and learning experience. The following is a reflection of my own personal opinions, and nothing more.*

Legacy software sometimes gets burried in layers of newer software.

That's certainly a good thing, becuase changing what's underneath can be risky. In the case of physics simulation code, obscure and disastrous 
...
- Linux-x64: Clone the source repository above and run <code>build.sh</code>. Depends on <code>libglew-dev</code> and <code>libglfw3-dev</code>.
...
![Monochromator](/assets/04_mono.png){: style=""}
...
Below is a tentative list of the notable features implemented in this project. It is separated into three sections, showcasing the simulation core, visualization and code structural aspects.
...
- particle trace "events" hooks: A callback for STATE, SCATTER, ABSORB
...
- monitor 2D data to texture using color maps and z-max
...
- particle trace recording (SCAT/STATE/ABSORB events)
- particle trajectories bundled by non-biased/random component-reached
- view trajectories by selected component
...
- instrument configuration state is layed-out sequentially in a localized "arena" container
- containers also manage the lifetime of a simulation
- separate instrument configs are managed individually
...
- the program can occationally crash due to a library bug
...
[mctrace-win]: https://github.com/jakob-garde/mctrace/release/x64-win/mctrace.zip

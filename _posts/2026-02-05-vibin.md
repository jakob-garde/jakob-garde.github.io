...
Here's my take on LLM usage for coding - YMMV. First, let's take on the elephant in the room: Some LLMs are sleazy.

- The LLM tends to endlessly stroke our egos.
...
- The LLM can't do more than one thing at the time without being wrong.
...
- LLM's can't see the big picture.

According to the hype disclaimer above, LLMs never demonstrated any real sense of purpose. The bigger the scope of your prompt, the harder it becomes to get an practical answer. We need to hold the purpose ourselves, or get sidetracked by the LLMs own less-relevant suggestions.

- The LLM tends to paint us into a corner.

The more suggestions we accept, the more we ma end up in an LLM-generated box and find ourselves in a bad spot, unable to backtrack. Thus we need to start over, losing time.
